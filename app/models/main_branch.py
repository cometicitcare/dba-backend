...
class MainBranch(Base):
    """
    Represents the top-level branches in the organizational hierarchy.
    There are 3 main branches.
    """
    __tablename__ = "main_branches"

    mb_id = Column(Integer, primary_key=True, index=True)
    mb_code = Column(String(10), nullable=False, unique=True, index=True)
    ...
    mb_created_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    ...
    # Relationships
    province_branches = relationship("ProvinceBranch", back_populates="main_branch")
    users = relationship("UserAccount", back_populates="main_branch")
