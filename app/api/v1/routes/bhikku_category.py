...
def manage_bhikku_categories(
    ...
    if action == CRUDAction.CREATE:
        if not payload.data or not isinstance(payload.data, BhikkuCategoryCreate):
    ...
    if action == CRUDAction.UPDATE:
        ...
        if not payload.data or not isinstance(payload.data, BhikkuCategoryUpdate):
            raise validation_error(
        ...
        try:
            updated = bhikku_category_service.update_category(
                db, cc_id=payload.cc_id, payload=payload.data, actor_id=user_id
            )
        except ValueError as exc:
            message = str(exc)
